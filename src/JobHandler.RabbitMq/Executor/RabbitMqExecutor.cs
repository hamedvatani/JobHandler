...
namespace JobHandler.RabbitMq.Executor
{
    public class RabbitMqExecutor<T> : IExecutor<T>
    {
        ...
        public void StartExecution(Func<T, CancellationToken, FuncResult> executor,
            Action<T, List<FuncResult>>? failAction)
        {
            _consumer.Received += async (_, ea) =>
            {
                ...
                try
                {
                    ...
                    if (job == null)
                        FailJob(ea, FuncResult.Fail("Cant parse job"), failAction);
                    else
                    {
                ...
                {
                    if (job == null)
                        FailJob(ea, FuncResult.Fail(ex.Message), failAction);
                    else
        ...
        private void RetryJob(BasicDeliverEventArgs ea, FuncResult result)
        {
            ...
            List<FuncResult> list = new List<FuncResult>();
        ...
        private void FailJob(BasicDeliverEventArgs ea, FuncResult result, Action<T, List<FuncResult>>? failAction)
        {
            var props = ea.BasicProperties;
        ...
        private List<FuncResult> DeserializeFuncResultList(byte[] data)
        {
            ...
            return JsonConvert.DeserializeObject<List<FuncResult>>(str) ?? new List<FuncResult>();
        }
    }
}